...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=12cm]{LsdWork}}
...
%\section{Example 2 - Replicator Dynamics model}

%\subsection{Cobb-Douglas - 3D graphs}

%\subsection{Percolation models - Lattices}

%\subsection{Adaptive rule}

%\subsection{Neural Principal Components Analysis}
...
\begin{center}
\begin{tabular}{ccc}
 \fbox{\includegraphics[width=3.5cm]{start.pdf}} &  \fbox{\includegraphics[width=4.5cm]{modgrp}} & \fbox{\includegraphics[width=4cm]{newmodel.pdf}} \\
(a) & (b) & (c) \\

\end{tabular}
  \caption{Creating a model. Initial choice to access the model browser (a). Choose to create a model or a group (b). Details for a new model (c).}  
\end{center}

\label{fig:new}
\end{figure}%
 
If you are already running LMM and missed the initial window (table \ref{fig:new} (a) ), just choose the menu option \menu{Model / Browse Models}. In both cases you will be led to a module showing all models available, starting from the two groups pre-defined in the installation, example models and work in progress (figure \ref{fig:modelbrowser1}).

Using the mouse pointer or the arrow keys choose a group, e.g. ``Work in Progress'', and then uses the module's own menu \menu{Edit \ New Model/Group}. 

This choice will offer you the choice between creating a new group or insert a new model. Opting for the latter you will be requested a few details to identify the model. Firstly a model name and a version number, which will be used solely to label the model for users. The third field, the directory name, needs to be a non-existing directory and cannot contain spaces, punctuations, etc.

Confirming on \menu{Ok} will create a new directory (or a warning in case of error) with all the necessary elements required for an ``empty'' \LsD model program.  The most relevant is the file that LMM will use assuming it contains the code for the equations of the model. The filename is conventionally labelled as \tech{fun\_XXX.cpp}, where \tech{XXX} is the model directory name. Users may change the file name, but this is potentially dangerous since LMM considers only changes to the equation file to be included in the model. 
...
\subsection{Introduction to \LsD equations}
Users define in the equations of the model all the operations that need to be executed by
the model. Equations are pieces of code are associated to a label; during a simulation run, whenever the system decides to compute the value for a variable, it passes the control of the program to the equation file. Here the system search for the piece of code associated to the variable to be computed, and executes its lines, returning the the simulation (i.e. searching for the next variable to compute) at the end of the line of the equations for that variable. Therefore, each equation must necessarily indicate at the very least two basic elements: the name of the variable it refers to, and the final value to be used as result of the equation execution. In between is possible, of course, to place any number of intermediate lines containing commands, typically used to elaborate the value to be returned.

If LMM is not showing the the equation file, use the menu \menu{Model/Show Equations} to have LMM re-opening the equations' file. It is very important that you never open file for the equations using the menu \menu{File/Open File}. In fact, although this is not formally incorrect, there is the possibility that you edit the wrong file. In this case, the
equations' editing is not included in the \LsD model program, which keeps on using the old, un-edited, equations' file, and therefore the \LsD model program will not include them. When LMM is requested to show the equations' file it reads the name of the file used to create the \LsD model program, and therefore there is no risk of editing the wrong file.\tech{Users can modify the name of the equation file changing the content of the compilation model options, using the relative option in menu \menu{Model}.}
...
The line \code{EQUATION("X")} marks the beginning of the equation, that is, the start of the code that any variable labelled \lsd{X} in the model will begin to execute whenever it needs to compute a value. The line following the header  indicates where you could (and should) include a comment describing what the equation is supposed to do. The last line, \code{RESULT()}, indicates the end of the equation and must be assigned, within the round parentheses, the numerical value returned by the equation and assigned to the variable.
...
When an equation file is edited it must explicitly be saved, since the compiler does not read the editor window, but only the content of the file. If you try to compile without saving, the system asks whether you want to continue without saving, or if your want to save the current file before compiling. To save the equation file (menu \menu{File/Save} or shortcut \menu{Ctrl+s}) and, to compile,  use menu entry \menu{Model / Compile and Run} (shortcut 
\menu{Ctrl+r}). This will create (and execute) the new \LsD model program containing the new equation.

If the \LsD model program windows do not appear, and you have an error message instead, this means that you managed to put an error in your equation's code. Tell LMM to not run the old \LsD model program file (it is the one without the new equations) and read the newly appeared \menu{Compilation Results }window for indications on the probably line number where the error is located. See also the appendix \ref{sec:progr_err} (pg.
...
To load the original configuration we saved before running the simulation use menu \menu{File/Load} and choose
...
\lsd{MyObj} and double-click on the variable \lsd{X} (or use the arrow keys to highlight
and press Enter). Now the Browser is transformed as shown in fig. \ref{fig:variable}.

\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=9cm]{variable.pdf}}
...
\menu{Debug}, \menu{Save} and \menu{Run Time Plot}. Check on all the three of them and
...
\menu{Debug} option allows to control the internal computation of \lsd{X} during a simulation
...
values of \lsd{X} through the time steps\footnote{Being a very short simulation it is likely that before being able to see it the window containing the graph will be covered by the \LsD browser. Use the icons to highlight the window, or minimize the \LsD browser.}. 

A more precise presentation of the data produced during a simulation run is produced with
the Analysis of Results module. From the Browser, choose menu \menu{Data/Analysis
Results}. 

\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=13cm]{analysis.pdf}}
...
window is composed by three lists: \menu{Series Available}, \menu{Series Selected} and
...
button \menu{Statistics} and observe the \lsd{Log} window (search for its icon in the icon bar of the screen). You can see some descriptive statistics concerning the series selected.
...
(instead, in the expression above we request \code{V("minimum")} twice). Moreover, using
...
menu item \menu{Edit/Insert \LsD Script} and then choose the option \menu{V("...")}. In
the resulting window choose the desired index for the \code{v[...]} and the type label
for the value to request, that is \lsd{minimum}. Leave the other two options to the default values. Notice that the script highlights the different fields; you can quickly move the from one field to the next by pressing the Enter key. 

\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=6cm]{scriptv.pdf}}
...
shortcut pressing at the same time the key \menu{Control} and \menu{s}).
...
\menu{Don't run} to abort the process). A new window will contain the message indicating
...
\menu{File/Load} and choose the \LsD file (likely called \textbf{Sim1}. We need to add three elements to the model: parameters \lsd{minimum} and \lsd{maximums}, and
variable \lsd{X1}. 
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=6cm]{edit_var.pdf}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=13cm]{init.pdf}}
...
\lsd{X1}. Observing their graph we see that they have the same dynamics, but for the
range.


As a matter of exercise, we can produce a scatter-plot graph, where two values of the two variable at each time step correspond to a point. In the Analysis of Results window check the box \menu{Time Series} and the box for \menu{XY plot}. Insert in the \menu{Series Selected} list firstly the variable \lsd{X} and then \lsd{X1}. After clicking on the button \menu{Plot} a new window we ask you whether you want to generate a high-quality or low-quality graph; press \menu{No} and a new graph will appear\footnote{Scatter plot graphs are generated using an external package, Gnuplot. \LsD allows either to incorportate the graph as image within a \LsD graphical window, or to use a Gnuplot window. In the former case you loose some of the quality, but you can manage the window as any other \LsD window. Gnuplot windows are instead managed by its own commands. Test both options, to appreciate the difference.}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=13cm]{numobject.pdf}}
...
the parameters' values. Given the number of copies of \lsd{MyObj} now there are 10 copies
...
\lsd{minimum}. Check on the option \menu{Increasing}. In the box labeled ``Numerical data ...'' there are two entry cells: type in the top cell, marked as \menu{Start}, -100 and, in the second entry marked \menu{Step} enter 10. Press \menu{Ok} to confirm and exit. Now \lsd{minimum} is set to -100 for the parameter \lsd{minimum} in the first object, to -90 for the second, etc.

Set also \lsd{maximum} as starting from 100 and changing of -10 at each step. This will
make  Now the 10 objects  will compute their \lsd{X1} varying over different intervals.
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=6cm]{selection.pdf}}
...
Return to the Browser and run the simulation (\menu{Run/Run}. After that enter in
...
Let's modify our model where the \lsd{MyObj} we have used up to now is contained within
...
\lsd{Root}. But, as already said, it is better to never use \lsd{Root} to contain other
than objects.}. Place the Browser to show the \lsd{MyObj} and then choose menu item
\menu{Model/Insert New Parent}. In the resulting window type the name of the new object,
say \menu{AggregateObj}. After confirming, verify that your model now includes
...
Number of objects/All types of objects}), you will see that the window shows 10 copies 
the newly created \lsd{AggregateObj} object type. On the right of the line for the new object there is a text signaling that other objects are contained within those objects. Click on the text and the window will create new 10 lines, indicating the ``groups'' of \lsd{MyObj}'s contained in each copy of \lsd{AggregateObj}, and each group contains just one copy\footnote{Alternatively, you could type `2' into the entry for the hierarchical level to show and click on \menu{Updated}\footnotesize.}.

\LsD constraints each copy of an element, to have all the same structure, although the actual numerical content may obviously vary. We therefore are forced to have at least one descending object \lsd{MyObj} within each copy of \lsd{AggregateObj}, although the number of descending objects can vary.

You can change independently the number of copies for
each of the groups of \lsd{MyObj}'s contained within each of the 10 \lsd{AggregateObj}'s, or modify at
once all of them. Click on the number 1 of one of the lines for \lsd{MyObj}. A new window will appear, like that reported in figure \ref{fig:numobj}

\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=13cm]{numobj.pdf}}
...
However, in \LsD we don't make use of indexes, nor we need to know the number of copies before starting the computation.  From the programming viewpoint we have the problem of ensuring that each copy of \lsd{AverageRW} is computed using with all, and only, the values of \lsd{RandomWalk}
...
\begin{enumerate}
  \item define two counters, say \texttt{counter1} and \texttt{counter2}, both initially set equal to 0;
	...
	\item add the value of \lsd{RandomWalk} for the current object to \texttt{counter1};
...
repetition the cycle assigns to \code{cur} a different copy of \lsd{MyObj}. \code{cur} is
the equivalent for objects of what temporary variables are for numbers: a temporary
memory location containing an assigned value; instead of a numerical value \code{cur} contains a specific copy of an object. In programming language \code{cur} is a ``pointer'', that
is a variable that, instead of storing numerical values, can store a generalized data structure, in our case \LsD
objects\footnote{The pointer \code{cur}, as well as the numerical variables \code{v[i]},
...
the cycle, that is, the number of \lsd{MyObj}. The first line cumulates in \code{v[0]} a
more complicated stuff: \code{VS(cur,"RandomWalk")}. This command is another form of the \code{V("...")} family, returning the values of elements
(i.e. variables or parameters) of the model. The specificity of \code{VS("...")} is that
...
Notice that the way the LSM arranges the order of completion of the equations is independent of the order for the starting the computations of the variables. Therefore it is possible to use any rule for scanning the variables of the model; in practice, \LsD begins (attempting to) computing firstly the variables in the higher objects (\lsd{Root} is always the first). When the scanning encounters a variable that have been already updated, because another variable requested its new value, it does not re-compute equation.
...
see the second line in the window \menu{Compilation Results}. There should be a line number, indicating
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=15cm]{relqua.pdf}}
...
The new version of this variable does not modify the result produced by for the variable \lsd{Sales}. But, although obviously an equation can return a single value for its variable, the code for the equation may contain any C++ (and \LsD-specific) statement. In the example below we requested the equation for sales to write a value (the difference between the lagged and current sales) onto an element of the model, called \lsd{DiffSales}. We can now introduce a parameter in \lsd{MyObj} labelled \lsd{DiffSales} that will contain the sales' differences for each firm in the model.

It is worth to notice that the use of a \code{WRITE} command is, in general, non necessary, since a variable with a specialized equation may replaced it. The reason for using the command \code{WRITE(...)} is to speed up the implementation of the model, or the execution of simulation runs. This command needs also to be used with caution because it does not allow the automatic management of updating. In fact, being a parameter, \LsD does not ``know'' the time when \lsd{DiffSales} is overwritten; therefore, if the model were to use its values within other equations, it would be necessary to manually ensure that the value is used in the correct way (before or after it is updated by equation for \lsd{Sales}). In our case we do not run any risk, since the parameter is used on for statistical purposes and does not affect other results of the model. 
...
Leave the first option selected to generate a series across times steps. Choose \menu{Sum} as elaboration and type a new label, for example \lsd{SumDiffSales}. Pressing \menu{Ok} the new series will be generated. Plotting a graph of this new series it will be obvious that the sum of the differences is not null across the simulation steps, and therefore we need to modify the model to force the market dimension to remain constant.
...
Save this configuration with a different name, say \menu{sim2.lsd}, in order to not overwrite the previous one.


\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[height=7cm]{sales.pdf}}
...
On the face of it, one may argue that sales dynamics depend on quality of products, represented in our model by ``variables'' \lsd{RandomWalk}. However, in this configuration we kept the qualities constant, by fixing the parameters \lsd{maximum} and \lsd{minimum} to zero. Therefore, sales dynamics may be expected to be monotonous, or not? 
...
step, which equation's computation we want to observe. Open menu \menu{Run/Sim.Setting}
and write in the field \menu{Insert Debugger at:} a time step, say 10. After having
pressed \menu{Ok} move the \LsD Browser to show the content of \lsd{MyObj}; double-click
on \lsd{Sales} and check on the option \menu{Debug: ...} in the resulting option window
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[height=10cm]{debugger.pdf}}
  \caption{Debugger set on the variable \lsd{Sales} at the 10${th}$ time step.}
  \label{fig:debugger}
\end{figure}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[height=0.65cm]{debheader.pdf}}
...
\menu{Quit} aborts the simulation at current time step; \menu{Step} will continue the simulation until the next variable marked to be debugged is updated. For the moment don't use any of
these buttons.
...
change each time showing the content of a copy of object: \lsd{Root}, 
\lsd{AggregateObject} or one of the \lsd{MyObj}'s. The number of the copy and the
label of the object shown by the debugger is indicated in figure \ref{fig:debcopy} (to
reach this press 3 times the button \menu{Next} or the right arrow). This label shows the text \menu{Object
instance:} on the left the currently shown copy. Moreover, it shows also
all the labels (and instance indexes) of the objects ``above'' the shown one, beginning
with \lsd{Root}.

\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[height=1cm]{debcopy.pdf}}
  \caption{Debugger's label of the object's copy currently shown (the $1^{st}$ copy of \lsd{MyObj} in a group of 10).}
  \label{fig:debcopy}
\end{figure}
...
In our case, we interrupted the simulation when the \lsd{Sales} of the first \lsd{MyObj} has been completed. Therefore we will have that copies of \lsd{Sales} in the other \lsd{MyObj} will not be updated for the current time step, as well as all the \lsd{RelativeQuality}'s variables.
...
change. Actually, it is \lsd{WAverageRW} that does change. At time 9 this variable
has the value of 1537.11, as we can observe moving the debugger in \lsd{AggregateObj}
...
Complete the simulation run, clicking on the button \menu{Run} and open the Analysis of Results. As we mentioned before we wanted to understand why sales dynamics of some firms are not monotonous, by change sign even though their qualities are constant. We need to find a suitable format to explain why this happen.
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=12cm]{ansales.pdf}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[angle=270,width=12cm]{RandomWalk.pdf}}
...
\subsection{\LsD equations: the calling object \code{c}}
...
\subsection{Optimizing simulations: pointer \code{hook}}
...
\subsubsection{\menu{Match ()} (\code{Ctrl+p})}
As the command \menu{Match \{\}}, but concerning parentheses.

\subsubsection{\menu{Insert \{} and \menu{Insert \}} (\code{Ctrl+(} and \code{Ctrl+)}) }

Insert curly brackets in the text, useful for keyboard layouts missing these symbols.

\subsubsection{\menu{Wrap/Unwrap} (Ctrl+w)}
...
\subsubsection{\menu{Browse Models}}
...
\subsubsection{\menu{Compile and Run model}}
...
\subsubsection{\menu{Show Equation File}}
...
\subsubsection{\menu{Show Makefile}}
...
\subsubsection{\menu{Show Compilation results}}
...
\subsubsection{\menu{Show Description}}
...
\subsubsection{\menu{System compilation options}}
...
\subsubsection{\menu{Model compilation option}}
...
\subsubsection{\menu{Generate a 'NO WINDOW' makefile}}
...
\subsubsection{Button \menu{Observe}}
...
\subsubsection{Button \menu{Debug}}
...
\subsubsection{Button \menu{Help}}
Show the manual page for the log window.

\subsubsection{Button \menu{Copy}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=7cm]{model_structure.pdf}}
...
Option \menu{Save} will have the time series of the data produced by the element to be saved for analysis (see Analysis of Results). Values computed for elements with this option not marked will be discarded as soon as they are no longer necessary during a simulation run, so as to keep low memory requirements.
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[height=6cm]{obj_opt.pdf}}
...
\subsubsection{Menu item \menu{Model/Add a variable} (\menu{Ctrl+v})}
...
New variables are inserted by assigning a label and a number of lags. The label must be a text string without special characters, as spaces, quotes, etc. The labels must match the variable's equation headers, and the matching is case-sensitive (i.e. \lsd{X} is different from \lsd{x}).
...
\subsubsection{Menu item \menu{Add a parameter} (\menu{Ctrl+p})}
...
\subsubsection{Menu item \menu{Model/Add a function} (\menu{Ctrl+n})}
...
\subsubsection{Menu item \menu{Add a descending Obj.} (\menu{Ctrl+d})}
...
\subsubsection{Menu item \menu{Model/Insert a new parent} }
...
\subsubsection{Menu item \menu{Model/Ignore equation file controls} }
...
\subsubsection{Menu item \menu{Model/Upload equation file} }
...
\subsubsection{Menu item \menu{Model/Offload the equation file} }
...
\subsubsection{Menu item \menu{Model/Compare eq. files} }
...
\subsubsection{Menu item \menu{Model/Generate automatic documentation} }
...
\subsubsection{Menu item \menu{Model/Create report} }
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=7cm]{create_report.pdf}}
...
\subsubsection{Menu item \menu{Model/Create LaTex report} }
...
\subsubsection{Menu item \menu{Model/Find an element of the model} (\menu{Ctrl+f})}
...
\subsubsection{Menu item \menu{Data/Set number of objects} (\menu{Ctrl+o})}

This command allows to define the number of objects in the model. There are two options. The first option (\menu{All types of objecs}) allows to determine the number of objects for all the types of objects in the model. The second option (\menu{Only current type of objects}), permits to determine the number of objects only for the types of objects shown by the browser. This second option is more limited, but is faster, while the first option allows much greater flexibility. In particular, permits to determine separatedly different numbers for objects of the same types in different ``branches'' of the model. For example, in a model with objects \lsd{Market} containing sets of objects \lsd{Firm}, allow to define many \lsd{Market}'s, each containing a different number of \lsd{Firm}'s.

See the section on \menu{Set number of objects} for further detail on this module.

\subsubsection{Menu item \menu{Data/Init. values} (\menu{Ctrl+i})}
...
\LsD offers the \menu{Set all} interface that allows the user to set all the initial values for an element specifying an initialization function. This interface is extremely flexible, allowing to select which elements should be considered, and which not. For example, it is possible to set, say, increasing values every second element present in the model. See the section for the module \menu{Init. values} for further details.



\subsubsection{Menu item \menu{Data/Analysis of Results} (\menu{Ctrl+a})}

This command activates the module \menu{Analysis of Results} that allows the user to analyse the results from the just finished simulation, or from previously saved \LsD result files. \LsD results are stored as one series of values for each element of the model indicated to save its results. Each series contains as many values as many time steps the elements existed in the simulation run. 
...
See the section on the module for \menu{Analysis of Results} for details on the use of this module.

\subsubsection{Menu item \menu{Data/Save Results} (\menu{Ctrl+z})}
...
\subsubsection{Menu item \menu{Data/Data Browse} (\menu{Ctrl+b})}
...
\subsubsection{Menu item \menu{Run/Run} (\menu{Ctrl+r})}
...
\subsubsection{Menu item \menu{Run/Set sim. settings} (\menu{Ctrl+m})}
...
The result files saved during a multiple simulation runs are named after the name of the configuration file name, extended with the number of the pseudo-random seed generator name used. Therefore, any simulation  saved during a multiple run can be reproduced using the configuration and the seed indicated. For example, if the configuration contained in the file \lsd{MyConfig.lsd} is defined to run 10 simulations with initial seed 2, the simulation results will be stored in files named from \lsd{MyConfig\_2.res} to \lsd{MyConfig\_11.res}

The results files can be loaded into the Analysis of Results' module to be analysed. However, in general users are interested in the last time step value of a multiple simulation run. Therefore, the \LsD model program generated also a \LsD result file containing the very last value of each variable saved during the simulation runs for each of the runs executed. The ``time steps'' in these files actually refers to different simulation runs. These files are normal \LsD result files with extension \code{.tot}, and are named after the configuration file name plus the initial and final seed value.
...
\subsubsection{Menu item \menu{Run/Remove debug flags} }
...
\subsubsection{Menu item \menu{Run/Remove save flags} }
...
\subsubsection{Menu item \menu{Run/Remove plot flags} }
...
\subsubsection{Menu item \menu{Run/Show elements saved} }
...
\subsubsection{Menu item \menu{Run/Show elements to observe} }

Modellers can mark some elements of the model as series relevant to be analysed. This option is independent from the indications of which elements are actually saved during a simulation run, so that a modeller may indicate with the \menu{observe} flags the meaningful elements for the model, which setting a different set of values to be saved, e.g. for optimization purposes only few data may be saved.
...
\subsubsection{Menu item \menu{Run/Show elements to initialize} }
...
\subsubsection{Menu item \menu{Run/Remove run time plots} }
...
\subsection{Module \menu{Set Objects' number}}
...
However, the module offers the possibility to modify not only the number of one group of objects of a given type, but several groups of that object in different ``branches'' of the tree of objects. Suppose your model includes 100 copies for \lsd{Market}, each containing 100 copies of \lsd{Firm}. Suppose that now you want to increase the number of \lsd{Firm}'s to 200, but not only in a single copy of \lsd{Market}, but in all of them. Using the default option, one may need to repeat the increment of object 100 times, once for each copy of \lsd{Market}. Obviously, this operation would be long and tedious, besides next to impossible in case you have, say, 10000 copies of \lsd{Market}. However, the user may simply tell the system that the new number of copies for \lsd{Firm} should be applied not only to the specific copy of \lsd{Market} selected, but to all the copies of \lsd{Market} present in the model. This feature is the more useful the more complex a hierarchical tree of objects is in the model


\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=6cm]{objnum1.pdf}}
...
In fact, the interface determining the scope of the changes allows to choose not only among the objects one layer above, as in the example case, but to any possible higher layer. For example, suppose that \lsd{Market} is defined as contained in objects of type \lsd{Country}. One may decide whether the change should be applied to a single \lsd{Market}; to all \lsd{Market}'s in a single \lsd{Country}; or to all \lsd{Market}'s in all \lsd{Country}'s. 
...
Note that the sequential number of a specific copy may be difficult to compute. For example, suppose you have a model with an object structure composed by objects \lsd{Country} containing \lsd{Market}'s containing finally \lsd{Firm}'s. Suppose that the configuration includes several copies of \lsd{Market}, each of them containing several copies of \lsd{Firm}'s. When increasing the number of objects \lsd{Firm}, normally, you may want to express the source copy in terms of the sequential index for copies of high level objects in their respective groups. For example, you may want to use as source for the new objects the third copy of \lsd{Firm} in the second \lsd{Market} in the fifth \lsd{Country}. To find our the sequential number of this copy requires the knowledge of the number of firms in each market and each country, and can easily produce an error. The interface offers the automatic computation of the sequential number for the source object in terms of the relative number for higher level objects. Pressing the button \menu{Compute} one may identify the sequential number automatically, and pass this value to the system. This feature is particularly useful in complex object structures, when the model includes many ``layers'' of objects.
...
\subsection{Module \menu{Initial values}}
...
The columns are identified with a ``tag'', a combination of digits referring to the ordinal copy of the objects containing the elements and all objects in higher layers. For example, a model containing objects \lsd{Market} in turn containing objects \lsd{Firm}, the columns for the initial values of elements in \lsd{Firm}'s will contain two digits: the first for the ordinal copy of \lsd{Market} and the second for the ordinal copy of \lsd{Firm} in that copy of market. A tag may for example be $12\_25$, indicating the initial value for the element indicated in the row and the copy contained in the $25^{th}$ copy of \lsd{Firm} among the set of this object contained in the $12^{th}$ \lsd{Market}. A similar model composed by objects \lsd{Country} containing \lsd{Markets}, then the initial values for \lsd{Firm} will be composed by three digits, as in $2\_12\_25$: $2^{nd}$ \lsd{Country}, $12^{th}$ \lsd{Market}, $25^{th}$ \lsd{Firm}.
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=4cm]{setall.pdf}}
...
The \menu{Set all} interface is composed by three sections: the nature of the initialization function; the frequency of its application; the extension of its application.
...
\menu{Equal to}. Assign the same value, as indicated in the first numerical cell, to all the copies of the element.
...
\menu{Increasing (group)}. This initialization function is identical to the \menu{Increasing} one, but it is re-set to the starting value at any ``jump'' of the elements to initialize. For example, consider a model composed by objects \lsd{Firm} contained in \lsd{Market}. Suppose there are many copies of \lsd{Market}, each containing many copies of \lsd{Firm}. Initializing an element contained in \lsd{Firm}, using this initialization function will assign increasing values to the set of elements contained in the first \lsd{Market}. Then, the initial value for the copy of the element in the first \lsd{Firm} contained in the second \lsd{Market} is reset to the starting value, equal to the value used for the first \lsd{Firm} in the first \lsd{Market}. 

\menu{Random (uniform)}. Generate a real number as a random value for each element to initialize, using a uniform random function whose limits are determined by the values in the data cells.

\menu{Random integer (uniform)}. Generate an integer number as a random value for each element to initialize, using a uniform random function whose limits are determined by the values in the data cells.

\menu{Random (normal)}. Generate a real number as a random value for each element to initialize, using a normal random function whose mean and deviation are those indicated in the data cells.
...
\subsection{Module \menu{Analysis of Results}}
...
The values that the user can process with this module are those produced during the last simulation run by elements marked as being saved for this purpose (see paragraph \ref{subsub:opt_ele}, pag. \pageref{subsub:opt_ele}). However, the module allows also to add further series in the set of the available series. Additional series can be obtained pressing button \menu{Add series}.


\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=4cm]{add_series.pdf}}
...
Thirdly, if the Analysis of Results module contains some series in the central box \menu{Series Selected}, the user can elaborate these series generating a single new series using the option \menu{Create series from selected series}.
...
This function generates a single new series, whose label and tag must entered by the user, computing one of the five available statistics on the series selected. The available statistics are: average, sum, max, min, and variance. There are two possible ways to compute the statistics: \menu{Computing over series} generates a series with the same number of time steps as those selected. In this case, each time step for the new series will be computed using the values from the series at the same time step. \menu{Computing over cases} generates instead a series with a number of fictitious time steps equal to the number of series used. Each series will be used to compute the statistics at a time step, in the order they appear in the box \menu{Series Selected}.
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=6cm]{sselection.pdf}}
...
\menu{Y2 axis}. This option is used only for time series sequential plots, and requires the use of the option \menu{Y self-scaling}. When this option is checked on the system will use two independent vertical axes. The user needs to specify which series must use the second vertical axis. All the previous series from that indicated will make use of the first vertical axis, while all the following ones will use the second. This plot allows to plot different series normalized on two independent scales.
...
Menu entry \menu{Color}. This menu allows to associate a color to each index number. This color will be used to differentiate the series according to their order.
...
Graphs can be modified adding labels, or editing existing ones. Labels can be added by keeping the key \menu{Shift} pressed and clicking on the location where the new label should appear. Right-clicking on an existing label it is possible to edit it, or delete it by assigning an empty text.

\subsubsection{Examples of graph types}

\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=10cm]{example_graph.pdf}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=10cm]{cross_section.pdf}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=10cm]{xy_cross.pdf}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=10cm]{example_scatter_time.pdf}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=10cm]{ex_histo.pdf}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=10cm]{ex_lattice.pdf}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=10cm]{ex_statistics.pdf}}
...
Pressing \menu{Continue} will generate the graph, while button \menu{Abort} terminates the window without generating the graph.

\subsubsection{Graph type \menu{Time Series - XY plot}}
...
\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=3cm]{phasedia.pdf}}
  \caption{\small When only one series is selected with options \menu{Time series} and \menu{XY Plot} the system generates a phase diagram plot, in which the values of the variables at time $t+k$ are plotted on the vertical axis, while the values of the same variable at time $t$ are plotted on the horizontal axis. The user must provide the $k$ value. Each value from 1 will generate an independent line.}
  \label{fig:phasedia}
\end{figure}
...
\subsubsection{Graph type \menu{Cross section - XY plot}}
...
\subsubsection{Graph type \menu{Histograms}}
...
\subsubsection{Graph type \menu{Lattice}}
...
The system can compute descriptive statistics of the values indicated. The statistics concern average, minimum, maximum, variance and standard deviations of the values indicated. The computation can be performed across time or across series, as indicated by the options \menu{Time series} or \menu{Cross section} respectively. The results will be shown in the log window.

\subsubsection{Exporting data}

Clicking on the \menu{Save data} button the system exports the values from the series selected in a text file. The exported data can be organized in several formats and using different types of headers, allowing also the specification of symbols for the missing data. 

\begin{figure}[ht]
  \centering
 \fbox{\includegraphics[width=3cm]{export.pdf}}
...
\subsection{Module \menu{\LsD Debugger} and \menu{Data Browser}}
...
\subsubsection{\code{if ... then ... else}}
...
\subsubsection{Use of cycle \code{for}} \label{fun:for}
...
\subsection{\LsD objects' links: \code{-$>$up}, \code{-$>$next} and \code{-$>$son}}
...
\subsubsection{\code{EQUATION("Label") ... RESULT(value)} }
...
\subsubsection{\code{V("X")} }\label{fun:val}
...
\subsubsection{\code{V\_CHEAT("X", fake\_caller)} }
...
\subsubsection{\code{STAT(X)} } \label{fun:stat}
...
\subsubsection{\code{WRITE("X",value)} }
...
\subsubsection{\code{INCR("X",value)} }
...
\subsubsection{\code{MULT("X",value)} }
...
\subsubsection{\code{SEARCH("X")} }\label{fun:search}
...
\subsubsection{\code{SEARCH\_CND("X",value)} }
...
\subsubsection{\code{RNDDRAW("X","Y")} }
...
\subsubsection{\code{CYCLE(obj,"ObjLabel")} }
...
variable for objects \code{obj}. In the normal form, the group of objects must be
...
\subsubsection{\code{SORT("ObjLabel","VarOrParLabel",DIRECTION)}}
...
\subsubsection{\code{INTERACT("message", value)}}
...
\subsubsection{\code{close\_sim()} function}
...
\subsubsection{Identifying faulty equations - \code{DEBUG\_AT(T)} }
